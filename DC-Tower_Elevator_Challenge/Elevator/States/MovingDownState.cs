...
namespace DC_Tower_Elevator_Challenge.Elevator.States
{
    public class MovingDownState : IElevatorState
    {
        ...
        {
            // collect passengers from higher floors that want to get down
            ...
            // move passengers down (and stop on the way if requested)
            while (this.Stops.Any(kv => this.Elevator.Floor >= kv.Key))
            {
        ...
        public void OnStop()
        {
            throw new InvalidOperationException("Elevator should not be stopping while moving down.");
        }
